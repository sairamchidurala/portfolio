      ...
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          {experiences.map((exp, index) => (
            <Card
              key={exp.title}
              className="p-8 bg-card-gradient border-border hover:border-primary/30 transition-all duration-500 hover:shadow-subtle-glow"
              style={{ animationDelay: `${index * 200}ms` }}
            >
              {/* Header */}
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
                <div>
                  <h3 className="text-card-title font-bold text-card-foreground mb-2">
                    {exp.title}
                  </h3>
